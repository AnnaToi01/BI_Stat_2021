...
```{r}
# df_medals <- df_ol %>% group_by(Team) %>% 
#   select(Medal, Team) %>% 
#   summarize(n_medals = n()) # %>% 
# #  arrange(-n) %>% 
# #  slice(1:3) %>% 
# # summarise_all(~paste(., collapse = '; '))
# df_part <- df_ol %>% group_by(Team) %>% 
#   select(Team) %>% 
#   summarize(n_participants = n())
# df_together <- cbind(df_medals, df_part[, 2]) %>% summarize(fraction = n_medals/n_participants)
# 
# 
# df_medals <- df_ol %>% group_by(NOC) %>% 
#   select(Medal, NOC) %>% 
#   summarize(n_medals = n())
...
Do people get fatter when they age? Let's compare age and weight for corresponding ages.